...
- The admin panel needs a second secret beyond `profiles.is_admin`: an admin passphrase,
  enrolled out of band with `npm run admin:passphrase -- invite --email you@example.com`
  and redeemed at `/admin/setup`. Without it `/admin` stays locked — see `docs/admin-security.md`.
