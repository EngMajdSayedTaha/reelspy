...
`lib/instagram/rate-limit.ts`; state in Postgres, mutated by atomic RPCs. Meta limits Business Discovery per **app**, and the ceiling **scales with the userbase**: ~200 calls/hour per connected user, one shared pool. The budget below tracks that live ceiling instead of a flat constant, so headroom grows as you add users — it is **not** a fixed pool users fight over.

| Guard | Default | Env |
|---|---|---|
| App-wide hourly token bucket (**dynamic**, Move 2) | `clamp(200 × connected_users × 0.7, 160, 5000)`, recomputed each `refresh-snapshots` run and stored on `meta_api_limiter.hourly_budget`; `consume_meta_quota` reads it in the same locked SELECT (no extra round-trip) and derives refill from it; falls back to the floor when unset | `META_HOURLY_BUDGET` (floor), `META_CALLS_PER_USER`, `META_BUDGET_SAFETY_FACTOR`, `META_MAX_HOURLY_BUDGET` (cap) |
| Per-user hourly cap | 80 (background/cron worker exempt — capped at the full app budget) | `META_USER_HOURLY_BUDGET` |
...
| Shared-pool gauge (`X-App-Usage %`) | last reading **decays linearly to 0 over the rolling hour** from `app_usage_at`, so the UI shows current headroom, not a frozen morning peak | `META_USAGE_DECAY_SECONDS` |
...
- TikTok/YouTube posts default **private** until each platform's app audit passes (`TIKTOK_ALLOW_PUBLIC` / `YOUTUBE_ALLOW_PUBLIC`). TikTok pull-from-URL additionally needs a domain-verified host; a verified custom domain on the R2 bucket (`R2_PUBLIC_BASE_URL`) is the primary route, and `presignTikTokUrl` (`lib/storage/r2.ts`) falls back to a free alternative when that isn't set — routing TikTok's pull through our own already-verified app origin via `app/api/publishing/media-proxy/route.ts` instead. IG/FB/Threads post publicly with no review.
