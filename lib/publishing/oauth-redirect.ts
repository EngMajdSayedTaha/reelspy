...
// The OAuth redirect URI each publishing provider (TikTok, YouTube, Threads)
// returns to after its consent screen. Like Meta's getMetaRedirectUri, this must
// be an EXACT, pre-registered string and must be identical in the authorize step
...
// THREADS_REDIRECT_URI still override for local dev. Whatever this resolves to
// must be registered in the provider console (TikTok redirect URI / Google
// authorized redirect URI / Threads "Redirect Callback URLs").
const ENV_KEY: Record<OAuthPlatform, string> = {
  tiktok: "TIKTOK_REDIRECT_URI",
  youtube: "YOUTUBE_REDIRECT_URI",
  threads: "THREADS_REDIRECT_URI",
};

export function getSocialRedirectUri(platform: OAuthPlatform): string {
